...
public class CloseRangeBadGuy2 : MonoBehaviour {
	...
	void Update () {
		...
		if ((this.transform.position - player.transform.position).magnitude <= distanceBeforeSeen) {
			...
			if (!GetComponent<Animator> ().GetBool ("Startled") && !GetComponent<Animator> ().GetBool ("Attacking")) {
		...
		if (Mathf.Abs((player.transform.position - this.transform.position).magnitude -distanceBeforeSeen) <  0.1) {
			following = false;
			confused = Time.time;
